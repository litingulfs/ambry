...
public class ClusterMapUtils {
  ...
  static class PartitionSelectionHelper implements ClusterMapChangeListener {
    ...
    private boolean isPartitionEligibleForParanoidDurability(PartitionId partitionId) {
      // Create a map where the keys are names of remote data centers and the values are all initialized to zero.
      // Would it be possible to add another filter that removes the replicas that are not in the required state?
      // Could we have a method that returns all the replicas in the required state and then we subtract them somehow?
      Map<String, Integer> validRemoteReplicasPerDc = partitionId.getReplicaIds().stream()
          ...
          .collect(Collectors.groupingBy(replica -> replica.getDataNodeId().getDatacenterName(),
              Collectors.reducing(0, replica -> 0, Integer::sum)));

      if (validRemoteReplicasPerDc.isEmpty())
        return false;

      // For each remote data center, count the number of replicas that are not down and also in either LEADER or STANDBY state.
      for (String dc : validRemoteReplicasPerDc.keySet()) {
        Set<ReplicaId> eligibleReplicas = new HashSet<>();
        EnumSet.of(ReplicaState.STANDBY, ReplicaState.LEADER)
            .forEach(state -> eligibleReplicas.addAll(partitionId.getReplicaIdsByState(state, dc)));

        for (ReplicaId r : eligibleReplicas) {
          if (!r.isDown()) {
            validRemoteReplicasPerDc.put(dc, validRemoteReplicasPerDc.get(dc) + 1);
          }
        }
      }
